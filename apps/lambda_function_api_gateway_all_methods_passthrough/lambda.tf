...
resource "aws_lambda_function" "app" {
  ...
  timeout = "${var.timeout}"
  vpc_config {
    subnet_ids = flatten(["${split( ",", var.private == 1 ?   join(",", module.aws_core_data.private_subnets) :  join(",", concat(module.aws_core_data.private_subnets,module.aws_core_data.public_subnets)))}"])
