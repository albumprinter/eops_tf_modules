...
resource "aws_lambda_function" "app" {
  ...
  timeout = "${var.timeout}"
  vpc_config {
    subnet_ids = "${split( ",", var.private == 1 ?   join(",", module.aws_core_data.aws_subnets.private.ids) :  join(",", concat(module.aws_core_data.aws_subnets.private.ids,module.aws_core_data.aws_subnets.public.ids)))}"
