...
resource "aws_lambda_function" "app" {
  ...
  timeout = "${var.timeout}"
  vpc_config {
    subnet_ids = var.private == 1 ? flatten([module.aws_core_data.private_subnets]) : concat(flatten([module.aws_core_data.private_subnets,module.aws_core_data.public_subnets]))
